...
class Diagram:
    ...
    def save(self,filenameOrFile):
        ...
        try:
            if file is None:
                file = open(filenameOrFile,"w", encodinf="utf-8")
...
SVG_TEXT = """<text x="{x}" y="{y}" text-anchor="{width}" \
font-family="sans-self" font-size="{fontsize}">{text}</text>"""
SVG_SCALE = 20
...
class SvgText:
    def __init__(self, x, y, text, fontsize):
        ...
        y *= SVG_SCALE
        fontsize *= SVG_SCALE
        # **locals() = SVG_TEXT.format(x=x, y=y, text=text,fontsize=fontsize)
